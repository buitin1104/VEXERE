...
export const getProvinces = async () => {
  try {
    ...
    const uniqueProvinces = provinces?.filter(
      (province, index, self) =>
        index === self.findIndex((p) => p.value === province.value),
    );
...
export const getDistricts = async (value) => {
  try {
    ...
    const uniqueDistrict = distrcits.filter(
      (province, index, self) =>
        index === self.findIndex((p) => p.id2 === province.id2),
    );
...
export const getWards = async (value) => {
  try {
    ...
    const uniqueDistrict = distrcits.filter(
      (province, index, self) =>
        index === self.findIndex((p) => p.id3 === province.id3),
    );
